...
- API에는 사용자 인증 기능이 없습니다. 서버는 `127.0.0.1`에만 바인딩하고, 신뢰할 수 없는 네트워크나 인터넷에 공개하지 마세요.
- `.env`, 로컬 녹음, 빌드 결과는 Git에서 제외됩니다. 서비스 계정 JSON 같은 인증 파일도 저장소 밖에 두세요.
...
스펙트로그램 아래 청크 막대의 꼬리는 각 2초 윈도우가 브라우저에서 실제로 준비된 시점부터 응답을 받을 때까지를 표시합니다. 오디오 장치의 샘플 시계와 컴퓨터 시계 사이의 장시간 누적 차이는 막대 길이에 더하지 않고 진단값으로만 따로 기록합니다.

녹음을 **완료**한 뒤 **CSV 다운로드**를 누르면 화면에서 최근 60분이 지난 행까지 포함한 전체 세션 진단 CSV를 받을 수 있습니다. 장시간 실행 문제를 확인할 때는 다음 열을 비교하세요.

- `window_end_delay_ms`: 윈도우 콜백부터 응답 JSON을 읽을 때까지의 전체 지연
- `capture_clock_drift_ms`: 녹음 시작 시각으로 계산한 예상 윈도우 종료와 실제 콜백 사이의 누적 시계 차이
- `in_flight_at_dispatch`: 해당 윈도우를 보낼 때 이미 처리 중이던 요청 수
- `encode_ms`: 브라우저의 WAV 인코딩 시간
- `request_ms`: 로컬 백엔드 요청 왕복 및 응답 JSON 처리 시간
- `backend_upload_ms`, `backend_provider_ms`, `backend_normalization_ms`, `backend_collection_ms`: 서버의 업로드 저장, Cochl 처리, 결과 정규화, 로컬 수집 단계 시간
- `backend_total_ms`: 서버가 응답을 만들기 전까지의 전체 처리 시간

`capture_clock_drift_ms`만 서서히 커지고 `request_ms`와 `backend_total_ms`가 일정하다면 실제 API가 느려진 것이 아니라 두 시계의 차이가 누적된 것입니다. `backend_provider_ms`가 커지면 Cochl 요청 경로를, `backend_collection_ms`가 커지면 로컬 디스크·수집 경로를 먼저 확인하면 됩니다. `request_ms`만 커지고 `backend_total_ms`가 일정하면 브라우저, 로컬 HTTP 전송 또는 응답 처리 쪽을 확인하세요.
...
의존성 취약점과 Python 정적 분석은 다음 명령으로 다시 확인할 수 있습니다. 감사 도구는 애플리케이션 런타임 의존성이 아니므로 필요할 때 개발 환경에 설치합니다.

```bash
.venv/bin/python -m pip install --upgrade pip-audit bandit
.venv/bin/python -m pip_audit --requirement backend/constraints.txt --progress-spinner off
.venv/bin/python -m bandit -q -r backend/app scripts -x backend/tests
